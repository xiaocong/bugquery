...
def read_data(mgd, red, min_id):
    ...
    while record_count > 0: 
        ...
        for i in range(record_count): 
            ...
            if obj['sys_info'].has_key('android.os.Build.VERSION.RELEASE') and obj['sys_info'].has_key('android.os.Build.PRODUCT') and obj['sys_info'].has_key('ro.build.revision'):
                platform=obj['sys_info']['android.os.Build.VERSION.RELEASE']
                product=obj['sys_info']['android.os.Build.PRODUCT']
                revision=obj['sys_info']['ro.build.revision']
                pipe.sadd('i:p:%s'%platform, product)
                pipe.sadd('ids:%s:%s:%s'%(platform,product,revision),_id)
            else:
                continue

            if obj['category'] == 'ERROR':
                bugType=obj['type']
                pipe.sadd('b:t', bugType)
                pipe.sadd('b:t:%s'%bugType, obj['name'])
                pipe.sadd('ids:b', _id)
                pipe.sadd('ids:b:%s:%s' % (bugType, obj['name']), _id)
                pipe.sadd('ids:b:%s' % bugType, _id)
                if(bugType=='CALL_DROP'):
